#include <vector>
#include <random>
#include <iostream>

#include "gate.h"
#include "wire.h"

Gate::Gate(int num_inputs, Wire* output) 
	: m_output(output), m_inputs(num_inputs), m_delay(0), m_current_state('X') {
    
}

Gate::~Gate(){ }

void Gate::wireInput(unsigned int id, Wire* in) {
  if(id < m_inputs.size()) {
    m_inputs[id] = in;
  }
}

NotGate::NotGate(Wire* a, Wire* o) : Gate(1, o) {
  wireInput(0, a);
}

Event* NotGate::update(uint64_t current_time) {
  char state = 'X';
  Event* e = nullptr;

  char in = m_inputs[0]->getState();
  if (in == '0') {
    state = '1';
  } else if (in == '1') {
    state = '0';
  }
  if (state != m_current_state) {
    m_current_state = state;
    uint64_t next = current_time + m_delay;
    e = new Event{next, m_output, state};
  }
  return e;
}

And2Gate::And2Gate(Wire* a, Wire* b, Wire* o) : Gate(2,o) {
    wireInput(0,a);
    wireInput(1,b);
}

Event* And2Gate::update(uint64_t current_time) {
  char state = '1'; // initialize state
  Event* e = nullptr; // create event object e
	for(auto w : m_inputs) { // loop through wires
		char in = w->getState(); // get wire state
		if(in == '0') { // if wire is 0, break, need both 1
			state = '0';
			break;
		}
		else if(in == 'X') { // if wire is X, state is X
			state = 'X';
		}
	} 
  if(state != m_current_state) { // update current state
    m_current_state = state;
    uint64_t next = current_time + m_delay; // add some recollection of time
		e = new Event {next, m_output, state}; // fill in event
	}
  return e; // return event
}

Or2Gate::Or2Gate(Wire* a, Wire* b, Wire* o) : Gate(2,o) {
    wireInput(0,a);
    wireInput(1,b);
}

Event* Or2Gate::update(uint64_t current_time) {
  char state = '0';
  Event* e = nullptr;
	for(auto w : m_inputs) {
		char in = w->getState();
		if(in == '1') {
			state = '1';
			break;
		}
		else if(in == 'X') {
			state = 'X';
		}
	}
  if(state != m_current_state) {
    m_current_state = state;
		uint64_t next = current_time + m_delay;
		e = new Event {next,m_output,state};   
	}
  return e;
}


